...
char s[200],t[200];
int i,j,k,c,XX,YY;
double xyz[401][301],xx,yy,zz,Xfac,Yfac;
fp=fopen("xyz.grid","r");
for(i=300;i>-1;i--){
  for(j=400;j>-1;j--){
...
while(c!=EOF){
  ...
  xx = (t[22]-'0')*100.+(t[23]-'0')*10.+(t[24]-'0')*1.;
  xx += ((t[26]-'0')*10.+(t[27]-'0')*1.+(t[28]-'0')*.1+(t[29]-'0')*.01)/60.;
  yy = (t[15]-'0')*10.+(t[16]-'0')*1.;
  yy += ((t[18]-'0')*10.+(t[19]-'0')*1.+(t[20]-'0')*.1+(t[21]-'0')*.01)/60.;
  XX = (int)(xx*100.)-12150;
  Xfac = xx*100.-12150.-(double)XX;
  YY = (int)(yy*100.)-4650;
  Yfac = yy*100.-4650.-(double)YY;
  zz = xyz[(int)(xx*100.)-12150][(int)(yy*100.)-4650];
