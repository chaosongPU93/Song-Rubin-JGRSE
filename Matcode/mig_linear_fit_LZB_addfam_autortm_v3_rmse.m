% function mig_linear_fit_LZB_addfam_autortm_v3_rmse
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% and the histogram is not biased by these 6 migrations, so we try to use
% rmse direction ONLY to see if the result is changed. 
%
% Note that the migrations that fall into the angle range of 'indsw' might change;
% and ones have poor lf constraints 'indpoorlf' might change; but the ones have 
% large difference in offsets due to a different selection of propagation direction
% 'indlargediff' would NOT change
%
%
...
%     i=46;
    ...
    for j = 1: size(mighf,1)
        x0 = mighfdum(j,1);
        y0 = mighfdum(j,2);
        [newx,newy] = coordinate_rot(x0,y0,-(angrmse(i)-90),0,0);
        mighfdum(j,1) = newx;
        mighfdum(j,2) = newy;
    end
...
%     c.Label.String = strcat(num2str(trange(i,1)),' of HF',' (hr)');
    ...
    text(f.ax(1),0.84,0.85,strcat({'rate: '},rate),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    text(f.ax(1),0.84,0.80,strcat(num2str(angrmse(i)),{'{\circ}'}),'FontSize',10,...
         'unit','normalized','horizontalalignment','center'); 
    hold(f.ax(1),'off');
...
%     c.Label.String = strcat(num2str(trange(i,1)),' of LF',' (hr)');
    ...
    text(f.ax(2),0.84,0.85,strcat({'rate: '},rate),'FontSize',8,...
         'unit','normalized','horizontalalignment','center');
    text(f.ax(2),0.84,0.80,strcat(num2str(angrmse(i)),{'{\circ}'}),'FontSize',10,...
         'unit','normalized','horizontalalignment','center');
    hold(f.ax(2),'off');
...
%     end
    ...
    [tmpx,tmpy] = coordinate_rot(medxhf,medyhf,-(angrmse(i)-90),0,0);
    medallhf(i,1:4) = [medxhf medyhf tmpx tmpy];
    [tmpx,tmpy] = coordinate_rot(medxlf,medylf,-(angrmse(i)-90),0,0);
...
%     end
    ...
    [propdiv,~] = coordinate_rot(x0,y0,-(angrmse(i)-90),0,0);
...
%%% reposition
set(f123.ax(1), 'position', [ 0.1, 0.1, 0.5, 0.75]);
set(f123.ax(2), 'position', [ 0.65, 0.15, 0.35, 0.75]);
...
for i = 1: size(trange,1)
    [rotx1, roty1] = complex_rot(0, ranvechf98(i,2)-medallhf(i,3), -angrmse(i));
    [rotx2, roty2] = complex_rot(0, medallhf(i,3)-ranvechf98(i,1), -angrmse(i));
...
polarhistogram(theta(angrmse>0 & angrmse<=90),'binedges',binedge,'normalization','count',...
               'facec',[0 1 1],'facea',0.8); hold on
binedge = deg2rad(2.5: 5: 92.5 + 90);           
polarhistogram(theta(angrmse>90 & angrmse<=180),'binedges',binedge,'normalization','count',...
               'facec','k','facea',0.9);
binedge = deg2rad(2.5: 5: 92.5 + 180);           
polarhistogram(theta(angrmse>180 & angrmse<=270),'binedges',binedge,'normalization','count',...
               'facec',[1 96/255 0],'facea',0.8);
binedge = deg2rad(2.5: 5: 92.5 + 270);           
polarhistogram(theta(angrmse>270 & angrmse<=360),'binedges',binedge,'normalization','count',...
               'facec','k','facea',0.4);
ax = gca;
...
ax.Box = 'on';
ax.GridAlpha = 0.3;
text(ax,deg2rad(60),3.5,num2str(sum(angrmse>0 & angrmse<=90)),'HorizontalAlignment','center',...
     'FontSize',11);           
text(ax,deg2rad(130),3.5,num2str(sum(angrmse>90 & angrmse<=180)),'HorizontalAlignment','center',...
     'FontSize',11);           
text(ax,deg2rad(225),3.5,num2str(sum(angrmse>180 & angrmse<=270)),'HorizontalAlignment','center',...
     'FontSize',11);
text(ax,deg2rad(315),3.5,num2str(sum(angrmse>270 & angrmse<=360)),'HorizontalAlignment','center',...
     'FontSize',11); 
...
%%% save figure
print(f123.fig,'-dpdf',strcat(rstpath,'/autortmv3.prop_direction_map_allrmse',num2str(nfam),'.pdf'));
...
indsw = setdiff(find(angrmse>=220 & angrmse<=280),[1,26,27]); % [3,5,6,7,9,12,13,15,22,23,24,28,29,33,35,37,38,41,43,44,49,51,52]
...
% NE migs
% ones within angle range and excluding those at distant fams, eg. 002, 010
indne = setdiff(find(angrmse>=40 & angrmse<=100),[1,26,27]); % [2,10,11,18,21,30,34,46,47]
% ones have poor lf constraints
indpoorlf = sortself(sortself(:,2)>=3, 1); %[40,4,5,33,3,15,11,23,35,21,45,17,2,31,30,24,32,39,6,9,27,37,51,19,48,26,46,22,8,29,18,47]
% ones have large difference in offsets due to a different selection of propagation direction
indlargediff = [2,3,11,14,24,29,32,34,36,40,43,44,49,52];  %
...
[f,barsw,pdfxlocsw,pdfvalsw,barne,pdfxlocne,pdfvalne,lgd] = ...
    plt_hist_combined_RTMs(indne,indsw,ranvechf98,medallhf,angrmse,xran,yran,vdistsw,wtsw,...
...
%%% save figure
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_rmse_mainSW+NE_all',num2str(nfam),'.pdf'));
...
[f,barisw,pdfxlocisw,pdfvalisw,barine,pdfxlocine,pdfvaline,lgd] = ...
    plt_hist_combined_RTMs(indne,indsw,ranvechf98,medallhf,angrmse,xran,yran,ivdistsw,iwtsw,...
...
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_rmse_mainSW+NE_ind',num2str(nfam),...
      '.pdf'));
...
conf = 99;
[f,barsw,pdfxlocsw,pdfvalsw,barne,pdfxlocne,pdfvalne,lgd] = ...
    plt_hist_combined_RTMs(indne2,indsw2,ranvechf98,medallhf,angrmse,xran,yran,vdistsw2,wtsw2,...
...
%%% save figure
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_rmse_mainSW+NE_all_smalldiff',num2str(nfam),'.pdf'));
...
conf = 99;
[f,barisw,pdfxlocisw,pdfvalisw,barine,pdfxlocine,pdfvaline,lgd] = ...
    plt_hist_combined_RTMs(indne2,indsw2,ranvechf98,medallhf,angrmse,xran,yran,ivdistsw2,iwtsw2,...
...
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_rmse_mainSW+NE_ind_smalldiff',num2str(nfam),...
      '.pdf'));
...
[f,barelse,pdfxlocelse,pdfvalelse] = ...
    plt_hist_combined_RTMs_otherrtm(indelse,ranvechf98,medallhf,angrmse,xran,yran,vdistelse,wtelse,...
                           binw,conf,'int');
                                 
                       
%%% save figure
print(f.fig,'-dpdf',strcat(rstpath,'/autortmv3.vdist_rmse_otherrtm_all',num2str(nfam),'.pdf'));
