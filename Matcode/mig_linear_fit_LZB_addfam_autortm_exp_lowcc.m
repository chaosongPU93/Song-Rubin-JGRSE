% function mig_linear_fit_LZB_addfam_autortm_v3_exp_lowcc
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
% This is pretty much the same as 'mig_linear_fit_LZB_addfam_autortm_v3_exp'
% except using the location results of dectection with a much lower CC threshold
% (ccminhf = 0.1) to see if HF can be seen at earlier time in this case. NOTE here
% LF is unchanged, so use the same LF files as in 'mig_linear_fit_LZB_addfam_autortm_v3_exp'
%
%
...
trange(:,2) = trange(:,2)-0.2*3600;
trange(:,3) = trange(:,3)+0.2*3600;
...
%     c.Label.String = strcat(num2str(trange(i,1)),' of HF',' (hr)');
    ...
    text(f.ax(1),0.55,0.1,strcat('I', {', including earlier time'}),'FontSize',12,'unit','normalized',...
...
%     c.Label.String = strcat(num2str(trange(i,1)),' of LF',' (hr)');
    ...
    text(f.ax(2),0.55,0.1,strcat('I', {', including earlier time'}),'FontSize',12,'unit','normalized',...
         'horizontalalignment','center');
...
%         print(f.fig,'-dpdf',strcat(rstpath,'/LZB.',num2str(nfam),'autortmv3.sel1long.lowcc.nodcut.',...
%             num2str(trange(i,1)),'_',num2str(trange(i,2)),'-',num2str(trange(i,3)),...
%             '.',num2str(winlenhf),'.',num2str(ccminhf),'_',num2str(winlenlf),'.',num2str(lofflf),'.',...
%             num2str(ccminlf),'.pdf'));
%     elseif strcmp(fname, fname2)
%         print(f.fig,'-dpdf',strcat(rstpath,'/LZB.',num2str(nfam),'autortmv3.sel1long.lowcc.eq8kmdcut.',...
%             num2str(trange(i,1)),'_',num2str(trange(i,2)),'-',num2str(trange(i,3)),...
%             '.',num2str(winlenhf),'.',num2str(ccminhf),'_',num2str(winlenlf),'.',num2str(lofflf),'.',...
%             num2str(ccminlf),'.pdf'));
%     elseif strcmp(fname, fname3)
%         print(f.fig,'-dpdf',strcat(rstpath,'/LZB.',num2str(nfam),'autortmv3.sel1long.lowcc.eq12kmdcut.',...
%             num2str(trange(i,1)),'_',num2str(trange(i,2)),'-',num2str(trange(i,3)),...
%             '.',num2str(winlenhf),'.',num2str(ccminhf),'_',num2str(winlenlf),'.',num2str(lofflf),'.',...
%             num2str(ccminlf),'.pdf'));        
%     end
