...
%% for detection in accumulative hit counts            
...
for ifam = 1: length(nfampool)        
    tmp1 = hfmapcount(hfmapcount(:,end)==str2double(nfampool(ifam,:)),:);
    [dx, dy] = absloc2relaloc(tmp1(:,1),tmp1(:,2),loccont(ifam,1),loccont(ifam,2));
    hfrelacount = [hfrelacount; dx dy tmp1];
    
    tmp2 = lfmapcount(lfmapcount(:,end)==str2double(nfampool(ifam,:)),:);
    [dx, dy] = absloc2relaloc(tmp2(:,1),tmp2(:,2),loccont(ifam,1),loccont(ifam,2));
    lfrelacount = [lfrelacount; dx dy tmp2];
end

% set dist cutoff
distmaxhf = 8;
distmaxlf = 12;
disthf = sqrt(hfrelacount(:,1).^2+hfrelacount(:,2).^2);
distlf = sqrt(lfrelacount(:,1).^2+lfrelacount(:,2).^2);
hfrelacountcut = hfrelacount(disthf<=distmaxhf,:);
lfrelacountcut = lfrelacount(distlf<=distmaxlf,:);
    ...
    for nd=1:length(timoffrot(:,1))      % num of rows, also num of days
        ...
        if isfile(fname)
            ...
            for j = 1: ndet
                ...
                ccaddhf(objind,1:4) = allrst(j,end-3:end);
            end
        else
            continue
        end
    ...
    for nd=1:length(timoffrot(:,1))      % num of rows, also num of days
        ...
        if isfile(fname)
            ...
            for j = 1: ndet
                ...
                ccaddlf(objind,1:4) = allrst(j,end-3:end);
            end
        else
            continue
        end
...
%
fid = fopen(strcat(rstpath, '/evtloc.allfam.dcut.',SUFFIXhf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4f %.4f %.4f %.4f %.4e %.4e %10.3e %10.3f %d \n',...
        hfrelatimecut');
fclose(fid);

fid = fopen(strcat(rstpath, '/evtloc.allfam.dcut.',SUFFIXlf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4f %.4f %.4f %.4f %.4e %.4e %10.3e %10.3f %d \n',...
        lfrelatimecut');
fclose(fid);
...
fid = fopen(strcat(rstpath, '/evtloc.allfam.nodcut.',SUFFIXhf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4f %.4f %.4f %.4f %.4e %.4e %10.3e %10.3f %d \n',...
        hfrelatimenocut');
fclose(fid);

fid = fopen(strcat(rstpath, '/evtloc.allfam.nodcut.',SUFFIXlf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4f %.4f %.4f %.4f %.4e %.4e %10.3e %10.3f %d \n',...
        lfrelatimenocut');
fclose(fid);


%% Time evolution and compare with Peng et al. 2015, THIS CONTAINS duplicates from diff. fams 
% select a date to plot the general migration pattern
dateall = unique(hfrelatimecut(:,12));
selyr = '2005';
selhf = hfrelatimecut(hfrelatimecut(:,12)>=str2double(selyr)*1000,:);
mindate = dateall(find(dateall>=str2double(selyr)*1000,1,'first'));
timeint = selhf(:,12)-2005254+11;
timedec = selhf(:,13)/3600/24;
time = timeint+timedec;
selhf(:,17) = time;

sellf = lfrelatimecut(lfrelatimecut(:,12)>=str2double(selyr)*1000,:);
timeint = sellf(:,12)-2005254+11;
timedec = sellf(:,13)/3600/24;
time = timeint+timedec;
sellf(:,17) = time;

selhfpeng = selhf(selhf(:,17)<15,:);
sellfpeng = sellf(sellf(:,17)<15,:);

%%% define and position the figure frame and axes of each plot 
f2.fig = figure;
set(f2.fig,'Position',[scrsz(3)/10 scrsz(4)/10 0.48*scrsz(3) 0.75*scrsz(4)]);
nrow = 2;
ncol = 2;
...
hold(f2.ax(1),'on');
scatter(f2.ax(1),selhf(:,1),selhf(:,2), 7, selhf(:,end), 'filled','o');  %, 'MarkerEdgeColor', 'w')
...
hold(f2.ax(2),'on');
dumhf = sortrows(selhf,-17);
scatter(f2.ax(2),dumhf(:,1),dumhf(:,2), 7, dumhf(:,end), 'filled','o');
...
hold(f2.ax(3),'on');
scatter(f2.ax(3),sellf(:,1),sellf(:,2), 14, sellf(:,end), 'filled','o');  %, 'MarkerEdgeColor', 'w')
...
text(f2.ax(3),0.85,0.93,'LF','FontSize',15,'unit','normalized');
text(f2.ax(3),0.04,0.93,'c','FontSize',12,'unit','normalized','EdgeColor','k','Margin',2);
f2.ax(3).Box = 'on';
grid(f2.ax(3), 'on');
f2.ax(3).GridLineStyle = '--';
axis(f2.ax(3), 'equal');
axis(f2.ax(3),[-20 30 -20 20]);
...
hold(f2.ax(4),'on');
dumlf = sortrows(sellf,-17);
scatter(f2.ax(4),dumlf(:,1),dumlf(:,2), 14, dumlf(:,end), 'filled','o');  %, 'MarkerEdgeColor', 'w')
...
text(f2.ax(4),0.85,0.93,'LF','FontSize',15,'unit','normalized');
text(f2.ax(4),0.04,0.93,'d','FontSize',12,'unit','normalized','EdgeColor','k','Margin',2);
f2.ax(4).Box = 'on';
grid(f2.ax(4), 'on');
f2.ax(4).GridLineStyle = '--';
axis(f2.ax(4), 'equal');
axis(f2.ax(4),[-20 30 -20 20]);
f2.ax(4).XLabel.String = 'E (km)';
f2.ax(4).YLabel.String = 'N (km)';
hold(f2.ax(4),'off');
    
% %%% save figure
% print(f2.fig,'-depsc',strcat(rstpath,'/allfam.hflf.',num2str(selyr),'.',num2str(winlenhf),'_',...
