...
% Further modified by chaosong@princeton.edu, 2021/04/21
...
% 	     colo,cohi,npas,npol,tipe))  %#ok<DSPS>
...
% design a filter, return the transfer function
% Cutoff frequency Wn, specified as a scalar or a two-element vector. The cutoff
% frequency is the frequency at which the magnitude response of the filter is 1/sqrt(2).
% For digital filters, the cutoff frequencies must lie between 0 and 1, where 1 
...
if nargout>5
  ...
  % return the 512-point frequency response of the filter
  [H,F]=freqz(B,A,512,Fs);
  HABS2=abs(H).^2;    % amplitude square of complex response
end
...
% to make it zero-phase when the npass is 2, ie, running the filter forward and backward over the data
if npas==2
