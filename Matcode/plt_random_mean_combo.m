function f = plt_random_mean_combo(meanrand,refval,binw)
...
for i = 1: size(meanrand,2)
    ...
    prob(i) = sum(abs(meanrand(:,i))>=abs(refval(i))) /N * 100;
    text(ax,0.02,0.9,strcat({'Prob(abs>=meandiff) = '},sprintf('%4.3e',prob(i))),'fontsize',10,...
         'unit','normalized');
...
%     ylim(ax, [0 1.5*abs(refval)]);
    legend([p1,p2],{'diff of mean of randoms','diff of wted mean of data'},...
           'location','south');
    hold(ax,'off');
end
...
for i = 1: size(meanrand,2)
    ...
    ax.Box = 'on';
    grid(ax, 'on');
    histogram(ax,meanrand(:,i),'binw',binw,'facec',[0.6 0.6 0.6],'facea',0.8);  %,'normalization','pdf'
    p2 = plot(ax,[refval(i) refval(i)],[0 200],'b--','linew',1);
    p3 = plot(ax,[mmeanrand(i) mmeanrand(i)],[0 200],'k--','linew',1);
    plot(ax,[-refval(i) -refval(i)],[0 200],'b--','linew',1);
...
%     xlim(ax, [0 1.5*abs(refval)]);
    legend([p2,p3],{'diff of wted mean of data',...
