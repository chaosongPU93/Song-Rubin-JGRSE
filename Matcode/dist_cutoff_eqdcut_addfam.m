...
% 
% This script uses the same dist cutoff, rather than different ones as in
% 'dist_cutoff_addfam'
%
...
fid = fopen(strcat(rstpath, '/evtloc.all',num2str(nfam),'fam.eq',num2str(distmaxhf),'kmdcut.',SUFFIXhf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4f %.4f %.4f %.4f %.4e %.4e %10.3e %10.3f %d \n',...
        hfrelatimecut');
fclose(fid);

fid = fopen(strcat(rstpath, '/evtloc.all',num2str(nfam),'fam.eq',num2str(distmaxlf),'kmdcut.',SUFFIXlf),'w+');
fprintf(fid,'%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %d %.4f %.1f %.4f %.4f %.4f %.4f %.4f %.4e %.4e %10.3e %10.3f %d \n',...
        lfrelatimecut');
fclose(fid);
...
hffname = strcat(rstpath, '/evtloc.all',num2str(nfam),'fam.eq',num2str(distmaxhf),'kmdcut.',SUFFIXhf);
lffname = strcat(rstpath, '/evtloc.all',num2str(nfam),'fam.eq',num2str(distmaxlf),'kmdcut.',SUFFIXlf);
...
%   main_arrival_time  cent_of_win  avecc ccadd1 ccadd2 ccadd3 ccadd4 ampmax ampmin eng normeng famnum
%
fidhf = fopen(strcat(rstpath, '/evtloc.all',num2str(nfam),'fam.eq',num2str(distmaxhf),'kmdcutnodou.',SUFFIXhf),'w+');
fidlf = fopen(strcat(rstpath, '/evtloc.all',num2str(nfam),'fam.eq',num2str(distmaxlf),'kmdcutnodou.',SUFFIXlf),'w+');
